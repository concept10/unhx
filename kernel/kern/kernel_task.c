/*
 * kernel/kern/kernel_task.c — Kernel bootstrap and IPC smoke test for NEOMACH
 *
...
void kernel_task_init(void)
{
    ...
    if (kernel_task_ptr())
        serial_putstr("[NEOMACH] kernel task (task 0) created\r\n");
    else
        serial_putstr("[NEOMACH] ERROR: failed to create kernel task\r\n");
}
...
{
    serial_putstr("[NEOMACH IPC] beginning IPC smoke test...\r\n");
    ...
    if (!task_a || !task_b) {
        serial_putstr("[NEOMACH IPC] FAIL: could not create test tasks\r\n");
        return;
    }
    serial_putstr("[NEOMACH IPC] task_a and task_b created\r\n");
    ...
    if (kr != KERN_SUCCESS) {
        ...
        serial_putstr("[NEOMACH IPC] FAIL: could not allocate port name\r\n");
        return;
    }
    ...
    if (!port) {
        ipc_space_unlock(space_a);
        serial_putstr("[NEOMACH IPC] FAIL: could not allocate port\r\n");
        return;
    }
    ...
    ipc_space_unlock(space_a);
    serial_putstr("[NEOMACH IPC] port allocated in task_a (receive right)\r\n");
    ...
    if (kr != KERN_SUCCESS) {
        ...
        serial_putstr("[NEOMACH IPC] FAIL: could not allocate port name in task_b\r\n");
        return;
    }
    ...
    ipc_space_unlock(space_b);
    serial_putstr("[NEOMACH IPC] send right granted to task_b\r\n");
    ...
    if (kr != KERN_SUCCESS) {
        serial_putstr("[NEOMACH IPC] FAIL: mach_msg_send returned error ");
        serial_puthex((uint64_t)kr);
        serial_putstr("\r\n");
        return;
    }
    serial_putstr("[NEOMACH IPC] task_b sent message\r\n");
    ...
    if (kr != KERN_SUCCESS) {
        serial_putstr("[NEOMACH IPC] FAIL: mach_msg_receive returned error ");
        serial_puthex((uint64_t)kr);
        serial_putstr("\r\n");
        return;
    }
     ...
     */
    serial_putstr("[NEOMACH IPC] message received: ");
    ...
    {
        serial_putstr("[NEOMACH IPC] magic: ");
        serial_puthex((uint64_t)recv_msg.magic);
        serial_putstr(" (correct)\r\n");
        serial_putstr("[NEOMACH] Phase 1 complete. Mach IPC operational.\r\n");
    } else {
        serial_putstr("[NEOMACH IPC] FAIL: message contents mismatch\r\n");
        serial_putstr("[NEOMACH IPC] expected magic=0xDEADBEEF, got ");
        serial_puthex((uint64_t)recv_msg.magic);
        serial_putstr("\r\n");
    }
